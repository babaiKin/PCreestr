...
namespace PCreestr
{
    public partial class loginForm : Form
    {
        public loginForm()
        {
            ...
            tbUser.Text = "NikolaevN";
            tbPassword.Text = "Ybrjkfq4xtndthnsq";
        }

        #region Переменные
        readonly static string sDomain = "NCSM.local";
        readonly static string sDefaultOU = "OU=NN,DC=NCSM,DC=local";
        readonly static string sDefaultRootOU = "DC=NCSM,DC=local";
        //прописал напрямую в каждой функции
        //сюда textbox'ы не лезут :(
        //readonly static string sServiceUser = "NikolaevN";
        //readonly static string sServicePassword = "Ybrjkfq4xtndthnsq";
        ...
        {
            return new PrincipalContext(ContextType.Domain, sDomain, sDefaultRootOU, tbUser.Text, tbPassword.Text);
        }
        ...
        {
            return new PrincipalContext(ContextType.Domain, sDomain, string.IsNullOrEmpty(sOU) ? sDefaultOU : sOU, tbUser.Text, tbPassword.Text);
        }
...
/// все возможные функции для работы с AD
/// 1) Проверка имени пользователя и пароля
/// 2) Проверка срока действия учетной записи
/// 3) Проверка существования пользователя в AD
/// 4) Проверяет блокировку пользователя
/// 5) Получить указанного пользователя Active Directory
/// 6) Получить группу Active Directory
/// 7) Установка нового пароля
/// 8) Включить учетную запись пользователя
/// 9) Отключить учетную запись пользователя
/// 10) Установить признак истечения срока действия пароля
/// 11) Разблокировка заблокированного пользователя
/// 12) Создание нового пользователя Active Directory
/// 13) Создание новой группы Active Directory
/// 14) Добавляет пользователя в указанную группу
/// 15) Удаляет пользователя из указанной группы
/// 16) Проверка на вхождение пользователя в группу
/// 17) Возвращает список групп, в которых состоит пользователь
/// 18) Возвращает список авторизационных групп пользователя
/// 19) Получить базовый основной контекст
/// 20) Получить основной контекст указанного OU
 ...
 public class cAD
{
    ...
    readonly static string sDomain = "domain.local";
    readonly static string sDefaultOU = "OU=test,DC=domain,DC=local";
    readonly static string sDefaultRootOU = "DC=domain,DC=local";
